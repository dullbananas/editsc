...
structs = {
	'DirectoryEntry': (
		...
		'i' # Offset of chunk data from the start of file, in bytes
	),
	'ChunkHeader': (
	...
	'Block': (
		'B' # Type of block
		'B' # 4 low bits contain light level, 4 high bits contain block-specific data
	),
	'SurfacePoint': (
...
class Blocks:
	def __init__(self, block_values):
		block_objects = [
			Block(blocktype=t, blockdata=d)
			for (t, d) in block_values
		]
